...
from textsumm.models import UrlSummary
from textsumm.serializers import UrlSerializer
...
@csrf_exempt
def youtubeUrlApi(request,id=None):
    if request.method=='GET':
        if id:
            departments = UrlSummary.objects.get(urlid=id)
            departments_serializer = UrlSerializer(departments)
            return JsonResponse(departments_serializer.data, safe=False)

        departments = UrlSummary.objects.all()
        departments_serializer = UrlSerializer(departments, many=True)
        return JsonResponse(departments_serializer.data, safe=False)

    elif request.method=='POST':
        employee_data=JSONParser().parse(request)
        u_input=employee_data['urlAddress']
        print(u_input)
        print("url test")
        # youtube_video = "https://www.youtube.com/watch?v=A4OmtyaBHFE"
        youtube_video = u_input
        ...
        print(output1[0]['summary_text'])
        employee_data['urlTranscript']=result
        employee_data['urlSummary']=output1[0]['summary_text']
        print("***")
        print(output1[0]['summary_text'])
        print("***")

        department_serializer = UrlSerializer(data=employee_data) 
        ...
        department=UrlSummary.objects.get(urlid=department_data['id'])
        department_serializer=UrlSerializer(department,data=department_data)
        if department_serializer.is_valid():
            department_serializer.save()
            return JsonResponse("Updated Successfully!!", safe=False)
        return JsonResponse("Failed to Update.", safe=False)
        ...
        department=UrlSummary.objects.get(urlid=id)
        department.delete()
        return JsonResponse("Deleted Succeffully!!", safe=False)
...
def userDetailsApi(request,id=None):
    if request.method == 'GET':
        ...
        if department_data['password'] != "null":
            if departments:
...
def manualDetailsApi(request,id=None):
    if request.method == 'GET':
        if id:
            departments = ManualUserDetails.objects.get(username=id)
        ...
        departments = ManualUserDetails.objects.filter(username=department_data['username']).exists()
        if departments:
        ...
        if department_serializer.is_valid() and not departments:
            department_serializer.save()
            return JsonResponse(department_data['username'], safe=False)
        return JsonResponse("Failed",safe=False)
    
    elif request.method == 'PUT':
        department_data = JSONParser().parse(request)
        department=ManualUserDetails.objects.get(username=department_data['username'])
        ...
        department=ManualUserDetails.objects.get(username=id)
        department.delete()
        return JsonResponse("Deleted Succeffully!!", safe=False)
...
def createFile(request,id=None):
    if request.method == 'GET':
        ...
        if department_data['input_text_or_audio']=="Text":
        ...
        elif department_data['input_text_or_audio']=="Youtube_Url":
            u_input=department_data['url_link']
            print(u_input)
            print("url test")
            # youtube_video = "https://www.youtube.com/watch?v=A4OmtyaBHFE"
            youtube_video = u_input
            ...
            print(output1[0]['summary_text'])
            department_data['input_text']=result
            department_data['output_text']=output1[0]['summary_text']
            # print("***")
            # print(output1[0]['summary_text'])
            # print("***")

            # department_serializer = UrlSerializer(data=department_data) 
